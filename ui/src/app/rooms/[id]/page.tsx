...
import { useEffect, useCallback, useRef, useState } from 'react'
import { useParams, useRouter } from 'next/navigation'
import { ArrowLeft, Wifi, WifiOff, RefreshCw, Plus } from 'lucide-react'
import CodeEditor   from '@/components/editor/CodeEditor'
import ActiveUsers  from '@/components/editor/ActiveUsers'
import FileTabs     from '@/components/editor/FileTab'
import FileTree     from '@/components/editor/FileTree'
...
export default function RoomPage() {
  ...
  const { fetchRoomById, currentRoom, leaveRoom } = useRoomStore()
  const {
    files, activeFileId, openFileIds, language,
    activeUsers, isConnected, isReconnecting,
    setFiles, addFile, removeFile,
    openFile, closeFile, setActiveFile, updateFile,
  } = useEditorStore()
  ...
  const {
    sendFileChange, sendFileOpen,
    sendFileCreate, sendFileDelete,
    sendCursorMove,
  } = useCollab(roomId)
  ...
  useEffect(() => {
    ...
    roomApi.get(`/rooms/${roomId}/files`).then((res) => {
      setFiles(res.data)
      if (res.data.length > 0) {
        openFile(res.data[0].id)
        sendFileOpen(res.data[0].id)
      }
    })
  }, [user, roomId])
  ...
  const handleCreateFile = async (name: string, path: string) => {
    try {
      const res = await roomApi.post(`/rooms/${roomId}/files`, { name, path, content: '' })
      const newFile: RoomFile = res.data
      addFile(newFile)
      sendFileCreate(newFile)
    } catch (err: any) {
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1.5">
            {isReconnecting ? (
              <>
                <RefreshCw size={12} className="animate-spin" style={{ color: 'var(--color-warning)' }} />
              ...
              <>
                ...
                <span style={{ fontSize: '11px', color: 'var(--color-danger)', fontFamily: 'var(--font-mono)' }}>
                  Offline
                </span>
              </>
            )}
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          <FileTabs
            ...
            openFileIds={openFileIds}
            activeFileId={activeFileId}
            onTabClick={(id) => { setActiveFile(id); sendFileOpen(id) }}
            onTabClose={closeFile}
          />
          ...
          {activeFile ? (
            ...
            >
              <p style={{ fontFamily: 'var(--font-mono)', color: 'var(--color-text-muted)', fontSize: '13px' }}>
                No file open
              </p>
              <button
                onClick={() => setShowNewFileModal(true)}
